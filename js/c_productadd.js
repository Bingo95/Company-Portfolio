$(function() {
	...
	function save_data_param(prodname, unit, prodsort, prodpic) {
		...
		var proddes = $('#summernote').summernote('code');
	...
	function img_uplode() {
		...
		if(prodpic == undefined) {
			prodpic = "http://www.jidisoft.com/image/prodmenu_nopic.jpg";
			//保存图片
				...
				if(flag_val == 1) {
					$.modal({
						...
						buttons: [{
								text: "是",
								onClick: function() {
									window.location.href = "c_productadd.html?app_id=" + app_id;
								}
							},
							{
								text: "否",
								className: "warn",
								onClick: function() {
									window.location.href = "c_productlist.html?app_id=" + app_id;
								}
							},
						]
					});
