$(function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(flag_val == 1) {
					...
					$("#add_product").attr("href","c_productadd.html?app_id=" + appid);
					if(_data.length > 0) {
						...
						productlist.append(
							...
							"<a href='c_productadd.html?app_id="+appid+"' class='weui-btn weui-btn_primary width_50'>添加</a>" +
							"</div>"
						)
					}
	...
	function product_Editor() {
		$(".productmodify").click(function() {
			...
			window.location.href = "c_productmodify.html?prodid=" + prodid + "&app_id=" + app_id;
		})
	}
})